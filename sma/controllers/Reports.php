...
class Reports extends MY_Controller
{
    ...
    function getExpiryAlerts($warehouse_id = NULL)
    {
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select("image, product_code, product_name, quantity_balance, expiry")
                ->from('purchase_items')
...
//            }
    ...
    function getProductsReport($pdf = NULL, $xls = NULL)
    {
        ...
        if ($start_date) {
            ...
            $rp = "( SELECT ri.product_id, SUM( ri.quantity ) returnedQty, SUM( ri.subtotal ) totalReturns from " . $this->db->dbprefix('return_items') . " ri group by ri.product_id ) PReturns";
        }
        if ($pdf || $xls) {
            ...
            $this->datatables
				...
				COALESCE( PCosts.purchasedQty, 0 ) as PurchasedQty,
				COALESCE( PSales.soldQty, 0 ) as SoldQty,
				COALESCE( PReturns.returnedQty, 0 ) as ReturnedQty,
				COALESCE( PCosts.totalPurchase, 0 ) as TotalPurchase,
				COALESCE( PSales.totalSale, 0 ) as TotalSales,
				COALESCE( PReturns.totalReturns, 0 ) as TotalReturns", FALSE)
                ->from('products')
                ->join($sp, 'sma_products.id = PSales.product_id', 'left')
                ->join($pp, 'sma_products.id = PCosts.product_id', 'left')
                ->join($rp, 'sma_products.id = PReturns.product_id', 'left');
...
//            $sp = "( SELECT si.product_id, SUM( si.quantity ) soldQty, SUM( si.subtotal ) totalSale, s.date as sdate from " . $this->db->dbprefix('sales') . " s JOIN " . $this->db->dbprefix('sale_items') . " si on s.id = si.sale_id where s.date >= '{$start_date}' and s.date < '{$end_date}' group by si.product_id ) PSales";
//        } else {
            $pp = "( SELECT pi.product_id, SUM( pi.quantity ) purchasedQty, SUM( pi.subtotal ) totalPurchase, customer from " . $this->db->dbprefix('purchase_items') . " pi left join " . $this->db->dbprefix('purchases') . " on " . $this->db->dbprefix('purchases') . ".id = pi.purchase_id group by pi.product_id ) PCosts";
            $sp = "( SELECT si.product_id, SUM( si.quantity ) soldQty, SUM( si.subtotal ) totalSale from " . $this->db->dbprefix('sale_items') . " si group by si.product_id ) PSales";
//        }
        if ($pdf || $xls) {
            ...
            $this->datatables
				...
				COALESCE( PCosts.totalPurchase, 0 ) as TotalPurchase,                                
				COALESCE( PSales.totalSale, 0 ) as TotalSales,
				PCosts.customer as customer", FALSE)
                ->from('products')
                ->join($sp, 'sma_products.id = PSales.product_id', 'left')
                ->join($pp, 'sma_products.id = PCosts.product_id', 'left');
            // ->group_by('products.id');
...
//                ->select("date, reference_no, biller, customer, GROUP_CONCAT(CONCAT(" . $this->db->dbprefix('sale_items') . ".product_name, ' (', " . $this->db->dbprefix('sale_items') . ".quantity, ')') SEPARATOR '\n') as iname, grand_total, paid, payment_status", FALSE)
                ->select("date, jo_no, customer, CONCAT(" . $this->db->dbprefix('sale_items') . ".product_name, '__', " . $this->db->dbprefix('sale_items') . ".quantity) as iname", FALSE)        
                ->from('sales')
                ->join('sale_items', 'sale_items.sale_id=sales.id', 'left')
                ->join('warehouses', 'warehouses.id=sales.warehouse_id', 'left');

//            $this->db
//                ->select("date, jo_no, customer, GROUP_CONCAT(CONCAT(" . $this->db->dbprefix('sale_items') . ".product_name, '__', " . $this->db->dbprefix('sale_items') . ".quantity) SEPARATOR '___') as iname, SUM(grand_total), paid, (grand_total-paid) as balance, payment_status", FALSE)
////                ->select("jo_no, customer, SUM(grand_total) as total", FALSE)
//                ->from('sales')
//                ->join('sale_items', 'sale_items.sale_id=sales.id', 'left')
//                ->join('warehouses', 'warehouses.id=sales.warehouse_id', 'left')
...
//            }
            ...
            if (!empty($data)) {
                ...
                $this->excel->getActiveSheet()->SetCellValue('B1', lang('jo_no'));
...
//                $this->excel->getActiveSheet()->SetCellValue('I1', lang('payment_status'));
                ...
                foreach ($data as $data_row) {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('B' . $row, $data_row->jo_no);
                    $this->excel->getActiveSheet()->SetCellValue('C' . $row, $data_row->customer);
                    $this->excel->getActiveSheet()->SetCellValue('D' . $row, $data_row->iname);
...
//                $this->excel->getActiveSheet()->getColumnDimension('I')->setWidth(20);
            ...
            $this->datatables
                ->select("date, jo_no, customer, CONCAT(" . $this->db->dbprefix('sale_items') . ".product_name, '__', " . $this->db->dbprefix('sale_items') . ".quantity) as iname", FALSE)
//                ->select("jo_no, customer, SUM(grand_total)", FALSE)
                ->from('sales')
                ->join('sale_items', 'sale_items.sale_id=sales.id', 'left')
                ->join('warehouses', 'warehouses.id=sales.warehouse_id', 'left');
...
//            }
    ...
    function getSalesReport2($pdf = NULL, $xls = NULL)
    {
        $this->sma->checkPermissions('sales', TRUE);
        if ($this->input->get('product')) {
            $product = $this->input->get('product');
        ...
        if ($this->input->get('jo_no')) {
            $reference_no = $this->input->get('jo_no');
        } else {
            $reference_no = NULL;
        }
        if ($this->input->get('start_date')) {
        ...
        if ($pdf || $xls) {
            ...
            $this->db
                ->select("date, jo_no, customer, CONCAT(" . $this->db->dbprefix('sale_items') . ".product_name, '__', " . $this->db->dbprefix('sale_items') . ".quantity) as iname, COALESCE(" . $this->db->dbprefix('return_items') . ".quantity,0) as returned, grand_total as total", FALSE)
//                ->select("jo_no, customer, SUM(grand_total)", FALSE)
                ->from('sales')
                ->join('sale_items', 'sale_items.sale_id=sales.id', 'left')
...
//            }
            if ($jo_no) {
                $this->db->where('sales.jo_no', $jo_no);
            }
//            if ($start_date) {
//                $this->db->where($this->db->dbprefix('sales').'.date BETWEEN "' . $start_date . '" and "' . $end_date . '"');
//            }
            ...
            if (!empty($data)) {
                ...
                $this->excel->getActiveSheet()->SetCellValue('C1', lang('customer'));
                $this->excel->getActiveSheet()->SetCellValue('D1', lang('product_qty'));
                $this->excel->getActiveSheet()->SetCellValue('E1', lang('returned_qty'));
                $this->excel->getActiveSheet()->SetCellValue('F1', lang('grand_total'));
                $row = 2;
                $total = 0;
                $paid = 0;
...
//                    $this->excel->getActiveSheet()->SetCellValue('A' . $row, $this->sma->hrld($data_row->date));
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('C' . $row, $data_row->customer);
                    $this->excel->getActiveSheet()->SetCellValue('D' . $row, $data_row->iname);
                    $this->excel->getActiveSheet()->SetCellValue('E' . $row, $data_row->returned);
...
//                $this->excel->getActiveSheet()->SetCellValue('H' . $row, $balance);
            ...
            $this->load->library('datatables');
            $this->datatables
                ->select("date, jo_no, customer, CONCAT(" . $this->db->dbprefix('sale_items') . ".product_name, '__', " . $this->db->dbprefix('sale_items') . ".quantity) as iname, COALESCE(" . $this->db->dbprefix('return_items') . ".quantity, 0) as returned, grand_total", FALSE)
//                ->select("jo_no, customer, SUM(grand_total)", FALSE)
                ->from('sales')
                ->join('sale_items', 'sale_items.sale_id=sales.id', 'left')
                ->join('return_items', 'return_items.sale_item_id = sale_items.id', 'left');
...
//            }
            if ($reference_no) {
                $this->datatables->where('sales.jo_no', $reference_no);
            }
//            if ($start_date) {
//                $this->datatables->where($this->db->dbprefix('sales').'.date BETWEEN "' . $start_date . '" and "' . $end_date . '"');
//            }
...
//                ->select("" . $this->db->dbprefix('purchases') . ".date, reference_no, " . $this->db->dbprefix('warehouses') . ".name as wname, supplier, GROUP_CONCAT(CONCAT(" . $this->db->dbprefix('purchase_items') . ".product_name, ' (', " . $this->db->dbprefix('purchase_items') . ".quantity, ')') SEPARATOR '\n') as iname, grand_total, paid, " . $this->db->dbprefix('purchases') . ".status", FALSE)
                ->select($this->db->dbprefix('purchases') . ".date, reference_no, " . $this->db->dbprefix('warehouses') . ".name as wname, CONCAT(" . $this->db->dbprefix('purchase_items') . ".product_name, '__', " . $this->db->dbprefix('purchase_items') . ".quantity) as iname, grand_total, " . $this->db->dbprefix('purchases') . ".status", FALSE)
                ->from('purchases')
...
//                ->group_by('purchases.id')
            ...
            if (!empty($data)) {
                ...
                $this->excel->getActiveSheet()->SetCellValue('A1', lang('date'));
                $this->excel->getActiveSheet()->SetCellValue('B1', lang('po_jo'));
                ...
                foreach ($data as $data_row) {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('F' . $row, $data_row->status);
    ...
    function getCustomers($pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
            ...
            $this->db
                ->select($this->db->dbprefix('companies') . ".id as id, jo_no, company, name,count(" . $this->db->dbprefix('sales') . ".id) as total, COALESCE(sum(grand_total), 0) as total_amount", FALSE)
                ->from("companies")
                ->join('sales', 'sales.customer_id=companies.id')
                ->where('companies.group_name', 'customer')
                ->order_by('companies.company asc')
                ->order_by('sales.jo_no asc');
...
//                $this->excel->getActiveSheet()->SetCellValue('D1', lang('email'));
                $this->excel->getActiveSheet()->SetCellValue('C1', lang('total_sales'));
...
//                    $this->excel->getActiveSheet()->SetCellValue('D' . $row, $data_row->email);
                    $this->excel->getActiveSheet()->SetCellValue('C' . $row, $this->sma->formatNumber($data_row->total));
                    $this->excel->getActiveSheet()->SetCellValue('D' . $row, $this->sma->formatMoney($data_row->total_amount));
...
//                $this->excel->getActiveSheet()->getColumnDimension('G')->setWidth(15);
            ...
            $this->datatables
                ...
                ->where('companies.group_name', 'customer')
                ->group_by('sales.jo_no')
                ->add_column("Actions", "<div class='text-center'><a class=\"tip\" title='" . lang("view_report") . "' href='" . site_url('reports/customer_report/$1') . "'><span class='label label-primary'>" . lang("view_report") . "</span></a></div>", "jo_no")
                ->unset_column('id');
            echo $this->datatables->generate();
    ...
    /***Get Cost Local Materials Report***/


    function cost()
    {
        $this->sma->checkPermissions('services');
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['users'] = $this->reports_model->getStaff();
        $this->data['billers'] = $this->site->getAllCompanies('biller');
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('payments_report')));
    ...
    function getCost($pdf = NULL, $xls = NULL)
    {
        $this->sma->checkPermissions('customers', TRUE);
    ...
    function cost_report($user_id = NULL)
    {
        $this->sma->checkPermissions('customers', TRUE);
        if (!$user_id) {
            $this->session->set_flashdata('error', lang("no_customer_selected"));
