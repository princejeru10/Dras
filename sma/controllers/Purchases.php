...
class Purchases extends MY_Controller
{
    ...
    function getPurchases($warehouse_id = NULL)
    {
        ...
        $receive_link = anchor('purchases/receive/$1', '<i class="fa fa-edit"></i> ' . lang('purchases_receive'));
...
//                ->select("id, date, reference_no, customer, " . $this->db->dbprefix('warehouses')  . ".name as wh, status, grand_total")
//                ->from('purchases')
...
//        } else {
            $this->datatables
                // ->select("id, date, reference_no, supplier, status, grand_total, paid, (grand_total-paid) as balance, payment_status")
                ->select($this->db->dbprefix('purchases')  . ".id as purchase_id, date, reference_no, customer, " . $this->db->dbprefix('warehouses')  . ".name as wh, customer_po, supplier_po, status, grand_total") 
                ->from('purchases')
                ->join('warehouses','warehouses.id = purchases.warehouse_id','left');   
//        }
    ...
    function view($purchase_id = NULL)
    {
        ...
        $this->data['requested_by'] = $inv->requested_by;
...
//generate pdf and force to download 
    ...
    function add($quote_id = NULL)
    {
        ...
        $this->form_validation->set_rules('customer_po', $this->lang->line("customer_po"), 'required');
//        $this->form_validation->set_rules('supplier_po', $this->lang->line("supplier_po"), 'required');
        ...
        // $this->form_validation->set_rules('prf_no', $this->lang->line("prf_no"), 'required');
        // $this->form_validation->set_rules('jo_no', $this->lang->line("jo_no"), 'required');
//        $this->form_validation->set_rules('po_no', $this->lang->line("po_no"), 'required');
        $this->form_validation->set_rules('mrf_no', $this->lang->line("mrf_no"), 'required');
        ...
        if ($this->form_validation->run() == true) {
            ...
            $supplier_po = $this->input->post('supplier_po');
//            $po_no = $this->input->post('po_no');
            // $invoice_no = $this->input->post('invoice_no');
            // $prf_no = $this->input->post('prf_no');
            $jo_no = $this->input->post('reference_no');
                ...
                if (isset($item_code) && isset($item_net_cost) && isset($item_quantity)) {
                    ...
                    if (isset($item_tax_rate) && $item_tax_rate != 0) {
                        ...
                        if ($tax_details->type == 1) {
                            ...
                            //     $item_tax = $this->sma->formatDecimal((($item_net_cost) * $tax_details->rate) / (100 + $tax_details->rate));
                            //     $tax = $tax_details->rate . "%";
                            //     $item_net_cost -= $item_tax;
                            // } else {
                                $item_tax = $this->sma->formatDecimal((($item_net_cost) * $tax_details->rate) / 100);
                                $tax = $tax_details->rate . "%";
                            // }
                ...
                foreach ($items as $item) {
                    ...
                    if ($row->type == 'combo') {
                        ...
                        foreach ($combo_items as $citem) {
                            ...
                            if (isset($crow->details)) {
                                unset($crow->details);
                            }
                            if (isset($crow->product_details)) {
                                unset($crow->product_details);
                            }
...
//        $this->form_validation->set_rules('po_no', $this->lang->line("po_no"), 'required');
        ...
        $this->form_validation->set_rules('customer_po', $this->lang->line("customer_po"), 'required');
        $this->form_validation->set_rules('requested_by', $this->lang->line("requested_by"), 'required');
        $this->form_validation->set_rules('warehouse', $this->lang->line("warehouse"), 'required|is_natural_no_zero');
        // $this->form_validation->set_rules('supplier', $this->lang->line("supplier"), 'required');
...
//                 'supplier_po' => $supplier_po,
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->updatePurchase($id, $data, $products)) {
            ...
            $this->data['warehouses'] = $this->site->getAllWarehouses();
//            $this->data['ponumber'] = $this->site->getReference('po');
            $this->data['reference_no'] = $this->data['inv']->reference_no;
            // $this->data['invoice_no'] = $this->data['inv']->invoice_no;
            $this->data['mrf_no'] = $this->data['inv']->mrf_no;
            $this->data['requested_by'] = $this->data['inv']->requested_by;
            // $this->data['received_by'] = $this->data['inv']->received_by;
            $this->load->helper('string');
            $value = random_string('alnum', 20);
            $this->session->set_userdata('user_csrf', $value);
            $this->session->set_userdata('remove_pols', 1);
    ...
    function receive($id = NULL)
    {
        ...
        $this->form_validation->set_message('is_natural_no_zero', $this->lang->line("no_zero_required"));
        $this->form_validation->set_rules('rr_no', $this->lang->line("rr_no"), 'required');
         $this->form_validation->set_rules('supplier_po', $this->lang->line("supplier_po"), 'required');
        $this->form_validation->set_rules('invoice_no', $this->lang->line("invoice_no"), 'required');
        // $this->form_validation->set_rules('mrf_no', $this->lang->line("mrf_no"), 'required');
        // $this->form_validation->set_rules('warehouse', $this->lang->line("warehouse"), 'required|is_natural_no_zero');
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $item_net_cost = $this->sma->formatDecimal($_POST['net_cost'][$r]);
                $item_quantity = $_POST['quantity'][$r];
                $item_received = $_POST['received'][$r];
                $purchases_item_id = $_POST['purchases_item_id'][$r];
//                $item_supplier = $_POST['supplier_id'][$r];
                $item_option = isset($_POST['product_option'][$r]) && $_POST['product_option'][$r] != 'false' ? $_POST['product_option'][$r] : NULL;
                $item_tax_rate = isset($_POST['product_tax'][$r]) ? $_POST['product_tax'][$r] : NULL;
                $item_discount = isset($_POST['product_discount'][$r]) ? $_POST['product_discount'][$r] : NULL;
                $item_expiry = isset($_POST['expiry'][$r]) ? $this->sma->fsd($_POST['expiry'][$r]) : NULL;
                $quantity_balance = abs($item_quantity - $item_received);
//                var_dump($item_quantity);
//                var_dump($item_received);
//                var_dump($quantity_balance);
//                exit;
                ...
                if (isset($item_code) && isset($item_net_cost) && isset($item_quantity) && isset($quantity_balance)) {
                    ...
                    $tempItem = $this->purchases_model->getItemByID($purchases_item_id);

                    $item_received += $tempItem->received;

                    if ($item_received == $item_quantity){
                        $status = "complete";
                    }else if($item_received < $item_quantity){
                        $status = "partial";
                    }else{
                        $status = "pending";
                    }

//                    var_dump($item_received,$status);exit();
...
//            var_dump($pr);exit;
            ...
            $this->data['warehouses'] = $this->site->getAllWarehouses();
            // $this->data['ponumber'] = $this->site->getReference('po');
            $this->data['po_no'] = $this->data['inv']->po_no;
            $this->data['invoice_no'] = $this->data['inv']->invoice_no;
            // $this->data['mrf_no'] = $this->data['inv']->mrf_no;
            $this->data['received_by'] = $this->data['inv']->received_by;
            $this->data['received'] = $this->data['inv']->received;
//            $this->data['supplier_id'] = $this->data['inv']->supplier_id;
            $this->data['note'] = $this->data['inv']->note;
            $this->load->helper('string');
            $value = random_string('alnum', 20);
            $this->session->set_userdata('user_csrf', $value);
            $this->session->set_userdata('remove_pols', 1);
            $this->data['csrf'] = $this->session->userdata('user_csrf');
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('purchases'), 'page' => lang('purchases')), array('link' => '#', 'page' => lang('purchases_receive')));
            $meta = array('page_title' => lang('purchases_receive'), 'bc' => $bc);
            $this->page_construct('purchases/receive', $meta, $this->data);
        }
    }
    ...
    function suggestions()
    {
        ...
        // $supplier_id = $this->input->get(NULL, TRUE);
