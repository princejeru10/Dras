...
//        }
        ...
        $return_link = anchor('sales/return_sale/$1', '<i class="fa fa-reply"></i> ' . lang('return_sale'));
...
//        } else {
            $this->datatables
                ->select("id, date, reference_no, customer, sale_status, customer_po, note, grand_total")
                ->from('sales');
//        }
...
//        }
    ...
    function getReturns($warehouse_id = NULL)
    {
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select($this->db->dbprefix('return_sales') . ".date as date, " . $this->db->dbprefix('return_sales') . ".reference_no as ref, " . $this->db->dbprefix('sales') . ".reference_no as sal_ref, " . $this->db->dbprefix('return_sales') . ".customer, "  . $this->db->dbprefix('return_sales') . ".grand_total, " . $this->db->dbprefix('return_sales') . ".id as id")
...
//        print_r($this->datatables->generate());exit;
    ...
    function add($quote_id = NULL)
    {
        ...
        $this->form_validation->set_rules('mrf_no', lang("mrf_no"), 'required');
        $this->form_validation->set_rules('jo_no', lang("jo_no"), 'required');
        $this->form_validation->set_rules('customer', lang("customer"), 'required');
        $this->form_validation->set_rules('customer_po', lang("customer_po"), 'required');
...
//            $this->data['billers'] = $this->site->getAllCompanies('biller');
    ...
    function edit($id = NULL)
    {
        ...
        $this->form_validation->set_rules('customer', lang("customer"), 'required');
        $this->form_validation->set_rules('customer_po', lang("customer_po"), 'required');
        $this->form_validation->set_rules('prepared_by', lang("prepared_by"), 'required');
        $this->form_validation->set_rules('issued_to', lang("issued_to"), 'required');
//        $this->form_validation->set_rules('biller', lang("biller"), 'required');
        $this->form_validation->set_rules('sale_status', lang("sale_status"), 'required');
        $this->form_validation->set_rules('payment_status', lang("payment_status"), 'required');
...
//            $this->data['billers'] = ($this->Owner || $this->Admin) ? $this->site->getAllCompanies('biller') : NULL;
    ...
    function return_sale($id = NULL)
    {
        ...
        $this->form_validation->set_rules('reference_no', lang("reference_no"), 'required');
        $this->form_validation->set_rules('dr_no', lang("do_reference_no"), 'required');
        $this->form_validation->set_rules('returned_by', lang("returned_by"), 'required');
        $this->form_validation->set_rules('received_by', lang("received_by"), 'required');
//        $this->form_validation->set_rules('paid_by', lang("paying_by"), 'required');
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $item_option = isset($_POST['option_id'][$r]) && $_POST['option_id'][$r] != 'false' ? $_POST['option_id'][$r] : NULL;
                $item_type = $_POST['product_type'][$r];
                $item_code = $_POST['product_code'][$r];
                $item_name = $_POST['product_name'][$r];
                $item_net_price = $this->sma->formatDecimal($_POST['net_price'][$r]);
                ...
                if (isset($item_code) && isset($item_net_price) && isset($item_quantity)) {
                    ...
                    $subtotal = $this->sma->formatDecimal(($this->sma->formatDecimal($item_net_price * $item_quantity) + $pr_item_tax));
                    $products[] = array(
                        'sale_id' => $id,
                        ...
                        'net_unit_price' => $item_net_price,
                        'quantity' => $item_quantity,
                        'warehouse_id' => $wh_id, //$sale->warehouse_id,
                        'item_tax' => $pr_item_tax,
                        'tax_rate_id' => $pr_tax,
                        'tax' => $tax,
                        'discount' => $pr_item_discount,
    ...
    function suggestions()
    {
        ...
        if ($rows) {
            foreach ($rows as $row) {
                ...
                $row->option = $option;
                if ($opt->cost != 0) {
                    $row->cost = $opt->cost + (($opt->cost * $customer_group->percent) / 100);
                } else {
                    $row->cost = $row->cost + (($row->cost * $customer_group->percent) / 100);
                }
                $combo_items = FALSE;
                if ($row->tax_rate) {
                    $tax_rate = $this->site->getTaxRateByID($row->tax_rate);
