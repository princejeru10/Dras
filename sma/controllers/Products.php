...
//            . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete1' id='a__$1' href='" . site_url('products/delete/$1') . "'>"
            . lang('r_u_sure') . "</p><a class='btn btn-danger' id='delete' data-action='delete'>"
...
//                 ->select($this->db->dbprefix('warehouses_products') . ".product_id as productid, " . $this->db->dbprefix('products') . ".image as image, " . $this->db->dbprefix('products') . ".code as code, " . $this->db->dbprefix('products') . ".name as name, brand as brand, " . $this->db->dbprefix('categories') . ".name as cname, cost as cost, " . $this->db->dbprefix('warehouses_products') . ".quantity as quantity, unit, " . $this->db->dbprefix('warehouses_products') . ".rack as rack, alert_quantity," . $this->db->dbprefix('purchase_items') . ".quantity as purchaseqty", FALSE)
...
//                 ->where('purchase_items.status', "pending")
//                 ->group_by("warehouses_products.product_id");
//         } else {
//            $query = "Select x.id as productid, x.image as image, x.code as code, x.name as name, brand as brand,sma_categories.name as cname, cost as cost, unit, ABS(SUM(sma_purchase_items.quantity) - SUM(sma_purchase_items.received)) as pending, alert_quantity, y.tquantity from sma_products x  left join sma_categories on sma_categories.id = x.category_id left join sma_purchase_items on x.id = sma_purchase_items.product_id left outer join (select *, SUM(COALESCE(sma_warehouses_products.quantity,0)) as tquantity from sma_warehouses_products group by sma_warehouses_products.product_id) y on x.id = y.product_id group by x.id";
            $query = "(select *, SUM(COALESCE(sma_warehouses_products.quantity,0)) as tquantity from sma_warehouses_products group by sma_warehouses_products.product_id) as y";
            $this->datatables
//                ->select($this->db->dbprefix('products') . ".id as productid, " . $this->db->dbprefix('products') . ".image as image, " . $this->db->dbprefix('products') . ".code as code, " . $this->db->dbprefix('products') . ".name as name, brand as brand," . $this->db->dbprefix('categories') . ".name as cname, cost as cost, unit, ABS(SUM(" . $this->db->dbprefix('purchase_items') . ".quantity) - SUM(" . $this->db->dbprefix('purchase_items') . ".received)) as pending, alert_quantity", FALSE)
                ->select($this->db->dbprefix('products') . ".id as productid, " . $this->db->dbprefix('products') . ".image as image, " . $this->db->dbprefix('products') . ".code as code, " . $this->db->dbprefix('products') . ".name as name, brand as brand," . $this->db->dbprefix('categories') . ".name as cname, cost as cost, y.tquantity, unit, ABS(SUM(" . $this->db->dbprefix('purchase_items') . ".quantity) - SUM(" . $this->db->dbprefix('purchase_items') . ".received)) as pending, alert_quantity", FALSE)
                ->from('products')
                ->join('categories', 'products.category_id=categories.id', 'left')
                ->join('purchase_items', 'products.id=purchase_items.product_id', 'left')
                ->join($query, 'products.id=y.product_id', 'left outer')
//                ->where('purchase_items.status', "pending")
//                ->or_where('purchase_items.status', "partial")
        ...
        //var_dump($this->datatables->generate());exit();
...
//            }
    ...
    function edit($id = NULL)
    {
        ...
        if ($this->form_validation->run('products/add') == true) {
            ...
            $data = array('code' => $this->input->post('code'),
                'barcode_symbology' => $this->input->post('barcode_symbology'),
                'name' => $this->input->post('name'),
                'brand' => $this->input->post('brand') ? $this->input->post('brand') : lang("generic"),
                'type' => $this->input->post('type'),
                'category_id' => $this->input->post('category'),
                'subcategory_id' => $this->input->post('subcategory'),
                'cost' => $this->sma->formatDecimal($this->input->post('cost')),
    ...
    function duplicate($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true && $this->products_model->addProduct($data, $items, $warehouse_qty, $product_attributes, $photos)) {
            ...
            $this->data['combo_items'] = $product->type == 'combo' ? $this->products_model->getProductComboItems($product->id) : NULL;
            $this->data['product_options'] = $id ? $this->products_model->getProductOptionsWithWH($id) : NULL;
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('products'), 'page' => lang('products')), array('link' => '#', 'page' => lang('duplicate_product')));
            $meta = array('page_title' => lang('duplicate_product'), 'bc' => $bc);
            $this->page_construct('products/duplicate', $meta, $this->data);
        }
    }
    ...
    function getadjustments($pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
            ...
            if ($product) {
                $this->db->like('adjustments.product_id', $product);
            }
            ...
            if ($product) {
                $this->datatables->like('adjustments.product_id', $product);
            }
    ...
    function return_material()
    {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => lang('products')));
        $meta = array('page_title' => lang('products'), 'bc' => $bc);
        $this->page_construct('products/return_material', $meta, $this->data);
    }
