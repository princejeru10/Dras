    ...
    public function syncAdjustment($data = array())
    {
        if(! empty($data)) {
            if($data['type'] == 'subtraction') {
                if ($wh_qty_details = $this->getProductQuantity($data['product_id'], $data['warehouse_id'])) {
                    $balance_qty = $wh_qty_details['quantity'] - $data['quantity'];
                    $this->updateQuantity($data['product_id'], $data['warehouse_id'], $balance_qty);
                } else {
                    $balance_qty = 0 - $data['quantity'];
                    $this->insertQuantity($data['product_id'], $data['warehouse_id'], $balance_qty);
                }
                ...
                if ($data['option_id']) {
                    if ($op_wh_qty = $this->getProductWarehouseOptionQty($data['option_id'], $data['warehouse_id'])) {
                        $op_balance_qty = $op_wh_qty->quantity - $data['quantity'];
                        $this->db->update('warehouses_products_variants', array('quantity' => $op_balance_qty), array('option_id' => $data['option_id'], 'warehouse_id' => $data['warehouse_id']));
                        $this->syncVariantQty($data['option_id']);
                    } else {
                        $op_balance_qty = 0 - $data['quantity'];
                        $wh_pr_op = array('option_id' => $data['option_id'], 'product_id' => $data['product_id'], 'warehouse_id' => $data['warehouse_id'], 'quantity' => $op_balance_qty);
    ...
    public function reverseAdjustment($id)
    {
        ...
        if($adjustment) {
            if($adjustment->type == 'subtraction') {
                $wh_qty_details = $this->getProductQuantity($adjustment->product_id, $adjustment->warehouse_id);
                $balance_qty = $wh_qty_details['quantity'] + $adjustment->quantity;
                $this->updateQuantity($adjustment->product_id, $adjustment->warehouse_id, $balance_qty);
                ...
                if ($adjustment->option_id) {
                    if ($op_wh_qty = $this->getProductWarehouseOptionQty($adjustment->option_id, $adjustment->warehouse_id)) {
                        $op_balance_qty = $op_wh_qty->quantity + $adjustment->quantity;
                        $this->db->update('warehouses_products_variants', array('quantity' => $op_balance_qty), array('option_id' => $adjustment->option_id, 'warehouse_id' => $adjustment->warehouse_id));
                        $this->syncVariantQty($adjustment->option_id);
                    } else {
                        $op_balance_qty = $adjustment->quantity;
                        $wh_pr_op = array('option_id' => $adjustment->option_id, 'product_id' => $adjustment->option_id, 'warehouse_id' => $adjustment->warehouse_id, 'quantity' => $op_balance_qty);
                        $this->db->insert('warehouses_products_variants', $wh_pr_op);
                        $this->syncVariantQty($adjustment->option_id);
                    }
                ...
                if ($adjustment->option_id) {
                    if ($op_wh_qty = $this->getProductWarehouseOptionQty($adjustment->option_id, $adjustment->warehouse_id)) {
                        ...
                        $op_balance_qty = 0-$adjustment->quantity;
                        $wh_pr_op = array('option_id' => $adjustment->option_id, 'product_id' => $adjustment->option_id, 'warehouse_id' => $adjustment->warehouse_id, 'quantity' => $op_balance_qty);
                        $this->db->insert('warehouses_products_variants', $wh_pr_op);
                        $this->syncVariantQty($adjustment->option_id);
                    }
