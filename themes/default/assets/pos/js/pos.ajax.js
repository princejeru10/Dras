...
$('#posdiscount').focus(function () {
	...
	if(inclusive_tax_price_calculation == 1) {
		$('#prModal').on('change', '#ptax', function (e) {
			var row = $('#' + $('#row_id').val());
			var item_id = row.attr('data-item-id');
			if(positems[item_id].row.item_tax_method == 0) {
...
/* -----------------------
	 ...
	 * Show manual item addition modal 
	 ...
	 $(document).on('click', '#addItemManually', function (e) {
	 	...
	 	positems[mid] = {"id": mid, "label": mname + ' (' + mcode + ')', "row": {"id": mid, "code": mcode, "name": mname, "type": "manual", "quantity": mqty, "price": mprice, "tax_rate": mtax, "tax_method": 1, "qty": mqty, "discount": mdiscount, "serial": "", "option": false}, "combo_items": false, "tax_rate": mtax_rate, "options": false};
	 	localStorage.setItem('positems', JSON.stringify(positems));
	 	loadItems();
	 	$('#mModal').modal('hide');
	 	$('#mcode').val('');
...
function loadItems() {
	...
	if (localStorage.getItem('positems')) {
		...
		$.each(positems, function () {
			...
			product_discount += formatDecimal(parseFloat(item_discount * item_qty));
				...
				if (pr_tax !== false) {
					if (pr_tax.type == 1) {
						...
						if (item_tax_method == '0') {
							pr_tax_val = formatDecimal(((item_price - item_discount) * parseFloat(pr_tax.rate)) / (100 + parseFloat(pr_tax.rate)));
							pr_tax_rate = pr_tax.rate + '%';
							item_price -= pr_tax_val;
						} else {
							pr_tax_val = formatDecimal(((item_price - item_discount) * parseFloat(pr_tax.rate)) / 100);
							pr_tax_rate = pr_tax.rate + '%';
						}
		...
		var gtotal = formatDecimal(((total + product_tax + invoice_tax) - total_discount) + shipping);
