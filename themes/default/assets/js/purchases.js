...
// If there is any item in localStorage
    ...
    // If there is any item in localStorage
    if (localStorage.getItem('poitems2')) {
        loadItems();
    }
    ...
    $('#reset').click(function (e) {
        bootbox.confirm("Are you sure?", function (result) {
            if (result) {
                ...
                if (localStorage.getItem('poitems2')) {
                    localStorage.removeItem('poitems2');
                }
...
// save and load the fields in and/or from localStorage
    var $supplier = $('.rsuppliers'), $currency = $('#pocurrency');
    ...
    $('#mrfno').change(function (e) {
        localStorage.setItem('mrfno', $(this).val());
    });
    if (mrfno = localStorage.getItem('mrfno')) {
        $('#mrfno').val(mrfno);
    }
    $('#customer_po').change(function (e) {
        localStorage.setItem('customer_po', $(this).val());
    });
    if (customer_po = localStorage.getItem('customer_po')) {
        $('#customer_po').val(customer_po);
    }
    $('#supplier_po').change(function (e) {
        localStorage.setItem('supplier_po', $(this).val());
    });
    if (supplier_po = localStorage.getItem('supplier_po')) {
        $('#supplier_po').val(supplier_po);
    }
    $('#rr_no').change(function (e) {
        localStorage.setItem('rr_no', $(this).val());
    });
    if (rr_no = localStorage.getItem('rr_no')) {
        $('#rrno').val(rr_no);
    }
    $('#requested_by').change(function (e) {
        localStorage.setItem('requested_by', $(this).val());
    });
    if (requestedby = localStorage.getItem('requested_by')) {
        $('#porequestedby').val(requestedby);
    }
    ...
    $('#rrnote').redactor({
        ...
        changeCallback: function (e) {
            var v = this.get();
            localStorage.setItem('rrnote', v);
        }
    });
    if (rrnote = localStorage.getItem('rrnote')) {
        $('#rrnote').redactor('set', rrnote);
    }
    $supplier.on("select2-close",function (e) {
        localStorage.setItem('posupplier', $(this).val());
        $(this).parentsUntil('td').parent().find('#supplier_id').val($(this).val());
    });
...
// Order discount calcuation
    ...
    $(document).on('click', '.podel', function () {
        ...
        $('#gtotal').text(formatMoney(gtotal));
        if (count == 1) {
            $('#posupplier, .rsupplier').select2('readonly', false);
            //$('#pocurrency').select2('readonly', false);
        }
    ...
    /*  $(document).on('click', '#editItem', function () {
     ...
     var sp = $('#posupplier').val();
     if(sp == 1) {
     poitems[item_id].row.supplier1price = cost;
     } else if(sp == 2) {
     poitems[item_id].row.supplier2price = cost;
     } else if(sp == 3) {
     poitems[item_id].row.supplier3price = cost;
     } else if(sp == 4) {
     poitems[item_id].row.supplier4price = cost;
     } else if(sp == 5) {
     poitems[item_id].row.supplier5price = cost;
     }
     ...
     poitems[item_id].row.discount = $('#pdiscount').val() ? $('#pdiscount').val() : '',
    ...
    $(document).on('click', '.edit', function () {
        ...
        if (site.settings.tax1) {
            ...
            if (ds.indexOf("%") !== -1) {
                var pds = ds.split("%");
                if (!isNaN(pds[0])) {
                    item_discount = parseFloat(((cost) * parseFloat(pds[0])) / 100);
                } else {
            ...
            var pr_tax_val = item.tax_rate ? ((cost - item_discount) * parseFloat(item.tax_rate.rate)) / 100 : 0;
        }
        ...
        $('#old_qty').val(qty);
        $('#pcost').val(cost);
        $('#punit_cost').val(parseFloat(cost) + parseFloat(pr_tax_val));
        $('#old_cost').val(cost);
        $('#row_id').val(row_id);
        $('#item_id').val(item_id);
        $('#poption').select2('val', item.row.option);
    ...
    if (inclusive_tax_price_calculation == 1) {
        $('#prModal').on('change', '#ptax', function (e) {
            var row = $('#' + $('#row_id').val());
            var item_id = row.attr('data-item-id');
            if (poitems[item_id].row.item_tax_method == 0) {
                var pr_tax_rate = false, tax = $(this).val(), unit_cost = $('#prModal').find('#punit_cost').val();
                $.each(tax_rates, function () {
                    if (this.id == tax) {
                        pr_tax_rate = this;
                    }
                });
                var pr_tax_val = (unit_cost * parseFloat(pr_tax_rate.rate)) / (100 + parseFloat(pr_tax_rate.rate));
                $('#pcost').val(formatDecimal(unit_cost - pr_tax_val));
            }
        });
    }
...
function loadItems() {
    ...
    if (localStorage.getItem('poitems')) {
        ...
        $.each(poitems, function () {
            ...
            var purchases_item_id = item.row.purchases_item_id, product_id = item.row.id, item_type = item.row.type, combo_items = item.combo_items, item_cost = item.row.cost, item_qty = item.row.qty, item_bqty = item.row.qty-item.row.rcvd, item_expiry = item.row.expiry, item_tax_method = item.row.tax_method, item_ds = item.row.discount, item_discount = 0, item_option = item.row.option, item_code = item.row.code, item_name = item.row.name.replace(/"/g, "&#034;").replace(/'/g, "&#039;");
            //console.log(localStorage.getItem("action"));
            if(localStorage.getItem("poaction")=="receive"){
                // var item_rcvd = item.row.rcvd!=null ? item.row.rcvd : 0;
                var item_rcvd =  0;
            }
            ...
            tr_html = '<td><input name="product[]" type="hidden" class="rcode" value="' + item_code + '"><input name="purchases_item_id[]" type="hidden" class="purchases_item_id" value="' + purchases_item_id + '"><input name="product_name[]" type="hidden" class="rname" value="' + item_name + '"><input name="product_option[]" type="hidden" class="roption" value="' + item_option + '"><span class="sname" id="name_' + row_no + '">' + item_name + ' (' + item_code + ')' + (sel_opt != '' ? ' (' + sel_opt + ')' : '') + '</span> <i class="pull-right fa fa-edit tip edit" id="' + row_no + '" data-item="' + item_id + '" title="Edit" style="cursor:pointer;"></i></td>';
            ...
            if (localStorage.getItem('poaction')=="receive") {
                tr_html += '<td><input name="quantity_balance[]" type="text" class="form-control text-center rbqty" value="' + formatDecimal(item_bqty) + '"><input class="form-control text-center rquantity" name="quantity[]" type="hidden" value="' + formatDecimal(item_qty) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();"></td>';
                tr_html += '<td><input class="form-control text-center rreceived" name="received[]" type="text" value="' + item_rcvd + '" data-id="' + row_no + '" data-item="' + item_id + '" id="received_' + row_no + '" onClick="this.select();"></td>';
                //tr_html += '<td><div class="input-group"><input type="text" name="supplier[]" value="" class="form-control rsuppliers" id="supplier" style="width:100%;" placeholder="Select Supplier"><div class="input-group-addon" style="padding-left: 10px; padding-right: 10px;"><a href="#" id="removeReadonly"><i class="fa fa-unlock" id="unLock"></i></a></div></div><input type="hidden" name="supplier_id[]" value="" id="supplier_id" class="form-control"></td>';
            }else{
    ...
    //    order_discount = 0;
    //    total_discount = 0;
    //    $("#poTable2 tbody").empty();
    //    poitems = JSON.parse(localStorage.getItem('poitems2'));
    //    //console.log(poitems);
    //
    //    $.each(poitems, function () {
    //
    ...
    //        poitems[item_id] = item;
    //
    //        var product_id = item.row.id, item_type = item.row.type, combo_items = item.combo_items, item_cost = item.row.cost, item_qty = item.row.qty, item_rcvd = item.row.rcvd, item_bqty = item.row.quantity_balance, item_expiry = item.row.expiry, item_tax_method = item.row.tax_method, item_ds = item.row.discount, item_discount = 0, item_option = item.row.option, item_code = item.row.code, item_name = item.row.name.replace(/"/g, "&#034;").replace(/'/g, "&#039;");
    //
    //        var supplier = localStorage.getItem('posupplier'), belong = false;
    //
    //        if (supplier == item.row.supplier1) {
    ...
    //        tr_html += '<td class="text-right"><input class="form-control input-sm text-right rcost" name="net_cost[]" type="hidden" id="cost_' + row_no + '" value="' + item_cost + '"><span class="text-right scost" id="scost_' + row_no + '">' + formatMoney(item_cost) + '</span></td>';
    //        tr_html += '<td><input name="quantity_balance[]" type="hidden" class="rbqty" value="' + item_bqty + '"><input class="form-control text-center rquantity" name="quantity[]" type="text" value="' + formatDecimal(item_qty) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();"></td>';
    //        tr_html += '<td><input class="form-control text-center rreceived" name="received[]" type="text" value="0" data-id="' + row_no + '" data-item="' + item_id + '" id="received_' + row_no + '" onClick="this.select();"></td>';
    //        tr_html += '<td><div class="input-group"><input type="text" name="supplier[]" value="" class="form-control rsuppliers" id="supplier" style="width:100%;" placeholder="Select Supplier"><div class="input-group-addon" style="padding-left: 10px; padding-right: 10px;"><a href="#" id="removeReadonly"><i class="fa fa-unlock" id="unLock"></i></a></div></div><input type="hidden" name="supplier_id[]" value="" id="supplier_id" class="form-control"></td>';
    //        if (site.settings.product_expiry == 1) {
    //            tr_html += '<td><input class="form-control date rexpiry" name="expiry[]" type="text" value="' + item_expiry + '" data-id="' + row_no + '" data-item="' + item_id + '" id="expiry_' + row_no + '"></td>';
    //        }
    ...
    //    // Totals calculations after item addition
    //    var gtotal = ((total + product_tax + invoice_tax) - total_discount) + shipping;
    //    $('#rtotal').text(formatMoney(total));
    //    $('#rtitems').text((an - 1) + ' (' + (parseFloat(count) - 1) + ')');
    //    $('#rtds').text(formatMoney(total_discount));
    //    if (site.settings.tax1) {
    //        $('#rttax1').text(formatMoney(product_tax));
    //    }
    //    if (site.settings.tax2 != 0) {
    //        $('#rttax2').text(formatMoney(invoice_tax));
    //    }
    //    $('#rgtotal').text(formatMoney(gtotal));
    //    if (an > site.settings.bc_fix && site.settings.bc_fix != 0) {
    //        $("html, body").animate({scrollTop: $('#poTable2').offset().top - 150}, 500);
    //        $(window).scrollTop($(window).scrollTop() + 1);
    //    }
    //    //audio_success.play();
    //}
